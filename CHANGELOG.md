...
## 1.7.0-local.1 - 2026-07-10

+ [修复] 默认 Docker Compose 构建当前本地代码，并将配置持久化到 `data/config.json`。
+ [安全] Sentinel SDK 仅允许通过已验证 TLS 从 `sentinel.openai.com` 下载执行。
+ [安全] 注册状态流改用 Authorization Header，避免管理员密钥出现在 URL。
+ [修复] 修复前端 TypeScript 配置导入并更新必要的安全依赖。
