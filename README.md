...
cd chatgpt2api
cp .env.example .env
# 编辑 .env，填写 CHATGPT2API_AUTH_KEY；建议使用 openssl rand -hex 32 生成
docker compose up -d --build
```

默认 Compose 只构建并运行当前目录中的主应用，不会拉取 GitHub 的远程 ChatGPT2API 镜像。认证密钥从 `.env` 读取，运行配置持久化到 `data/config.json`。
...
其中 `caomingjun/warp`、`vimagick/privoxy` 和 `flaresolverr` 是可选的第三方镜像；不需要这条代理链路时请使用默认 `docker-compose.yml`。
...
cd chatgpt2api
uv sync
CHATGPT2API_AUTH_KEY=your_secret_key uv run main.py
```
...
如果直接通过公网 HTTP 暴露，功能可以正常工作，但 HTTP 无法防止管理员密钥和请求内容在传输途中被窃听。至少应使用长随机密钥并限制来源 IP；长期公网使用仍建议在前面增加 HTTPS 反向代理。
